...
//				}
	...
	BOOST_AUTO_TEST_CASE(RedistancingSussman_unit_disk_convergence_test_time)
	{
		...
		size_t N = 64;
//		double dt = 0.00100781; // N = 64
		double dt = 1.9073486328125e-06;
		double tf = 1e4 * dt;
		dt *= 4.0;
		for (int i = 0; i < 5; i++)
		{
...
//		const double box_upper = 1.0;
		...
		{
			auto key = dom.get();
//			g_dist.template get<Phi_0_grid>(key) = sgn(g_dist.getPos(key)[x]);
//			g_dist.template get<SDF_exact_grid>(key) = g_dist.getPos(key)[x];
			////------- TEST ----- INITIALIZE WITH SHIFTED COSINE------------///////////
			g_dist.template get<Phi_0_grid>(key) = cos(g_dist.getPos(key)[x] * M_PI);
			g_dist.template get<SDF_exact_grid>(key) = g_dist.getPos(key)[x] + 0.5;
			////------- TEST ----- INITIALIZE WITH SDF-----------------------///////////
//			g_dist.template get<Phi_0_grid>(key)     = g_dist.getPos(key)[x];
// 			g_dist.template get<SDF_exact_grid>(key) = g_dist.getPos(key)[x];
			++dom;
		}
		
//		double dt = 1 / (2.0 / (g_dist.getSpacing() * g_dist.getSpacing())) ;
		double dt = 4.76e-07 ;
		dt *= 2.0;
//		double tf = 1e5 * dt/2.0;
		for (int i = 0; i < 3; i++)
		{
			dt /= 2.0;
//			size_t max_iter = (size_t) std::round(tf / dt);
			...
			redist_options.max_iter = 1e12;
			redist_options.order_space_op    = 5;
			redist_options.order_timestepper = 3;
...
//			redist_options.convTolResidual.value = EPSILON;
			
			redist_options.interval_check_convergence = 100;
			redist_options.width_NB_in_grid_points = 16;
			redist_options.print_current_iterChangeResidual = true;
			redist_options.print_steadyState_iter = true;
			
			redist_options.id_checkpoint = N/2 + redist_options.width_NB_in_grid_points/2;
...
//			std::cout << "CFL dt = " << redist_obj.get_time_step() << std::endl;
			...
			vd_narrow_band.write(
					"vd_error_N" + std::to_string(N) + "_gradientOrder" + std::to_string(redist_options.order_space_op) +
					"_RKorder" + std::to_string(redist_options.order_timestepper), FORMAT_BINARY);
//				vd_narrow_band.save("test_data/output/vd_nb8p_error" + std::to_string(N) + ".bin");
			// Compute the L_2- and L_infinity-norm and save to file
			L_norms lNorms_vd;
			lNorms_vd = get_l_norms_vector<Error_vd>(vd_narrow_band);
			write_lnorms_to_file(dt, lNorms_vd, "l_norms_N" + std::to_string(N) + "_RKorder" + std::to_string(redist_options.order_timestepper),"./");
