...
BOOST_AUTO_TEST_SUITE(RedistancingSussmanTestSuite)
	...
	BOOST_AUTO_TEST_CASE(RedistancingSussman_unit_sphere)
	{
		...
		const double center[grid_dim] = {5.0, 5.0, 5.0};
		...
		redist_options.min_iter                             = 1191;
		redist_options.max_iter                             = 1191;     // max. number of iterations you want to run the redistancing, even if steady state might not yet have been reached (default: 1e6)
		...
		BOOST_CHECK(lNorms_vd.l2 <= 0.054358 + EPSILON && lNorms_vd.linf <= 0.109161 + EPSILON);
	}
