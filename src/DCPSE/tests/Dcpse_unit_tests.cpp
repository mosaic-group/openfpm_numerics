...
//                break;
    ...
    BOOST_AUTO_TEST_CASE(Dcpse_2D_test_without_cancellation)
    {
        bool check = true;
        const unsigned int accuracy = 5;
        std::cout << "r=" << accuracy << std::endl;
        double maxError[2] = {0.,0.};
        double rmse[2] = {0.,0.};
        double time[2] = {};
        unsigned int ifopt = 0;
        timer t;
        for ( auto fopt : {Formula_options::STANDARD, Formula_options::CANCELLATION})
        {
            t.start();
            int rank;
            MPI_Comm_rank(MPI_COMM_WORLD, &rank);
            ...
            BOOST_TEST_MESSAGE("Init vector_dist...");
            vector_dist<2, double, aggregate<double, double, double>,CartDecomposition<2,double>> domain(0, box, bc, ghost);
            ...
            //        std::cout << "rCut = " << rCut << std::endl;
            BOOST_TEST_MESSAGE("DCPSE init & compute coefficients...");
            Dcpse<2, vector_dist<2, double, aggregate<double, double, double>> > dcpse(domain, Point<2, unsigned int>({1, 0}), accuracy, rCut,2, support_options::N_PARTICLES, fopt);
            BOOST_TEST_MESSAGE("DCPSE compute diff operator...");
            dcpse.template computeDifferentialOperator<0, 1>(domain);
            ...
            for (auto itVal = domain.getDomainIterator(); itVal.isNext(); ++itVal)
            {
                auto key = itVal.get();
                computedValue = domain.template getProp<1>(key);
                validationValue = domain.template getProp<2>(key);
                double error=fabs(computedValue - validationValue);
                rmse[ifopt] += error*error;
                bool locCheck = (error < TOL);
                if(error > maxError[ifopt])
                {
                    maxError[ifopt] = error;
                }
                check = check && locCheck;
                if (!locCheck)
                {
            ...
            rmse[ifopt] = sqrt(rmse[ifopt]/numValues);
            if (fopt == Formula_options::STANDARD)
                std::cout << "Formula_options::STANDARD" << std::endl;
            else if (fopt == Formula_options::CANCELLATION)
                std::cout << "Formula_options::CANCELLATION" << std::endl;
            std::cout << "rmse=" << rmse[ifopt] << std::endl;
            std::cout << "Maximum Error (difference)=" << maxError[ifopt] << std::endl;
            
            t.stop();
            std::cout << "Elapsed time: " << t.getcputime() << " s\n";
            time[ifopt] = t.getcputime();
            ++ifopt;
        }
