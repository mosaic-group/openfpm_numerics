...
void indexUpdate(
        particle_type &Particles,
        particle_type2 &Particles_subset,
        openfpm::vector<aggregate<int>> &boundary, openfpm::vector<aggregate<int>> &boundary_without_corner,
        openfpm::vector<aggregate<int>> &corner_ul, openfpm::vector<aggregate<int>> &corner_ur,
        openfpm::vector<aggregate<int>> &corner_dl, openfpm::vector<aggregate<int>> &corner_dr,
        openfpm::vector<aggregate<int>> &bulk, Box<2, double> &up, Box<2, double> &down, Box<2, double> &left,
        Box<2, double> &right) {
    boundary.clear();
    boundary_without_corner.clear();
    corner_ul.clear();
    corner_ur.clear();
    corner_dl.clear();
    corner_dr.clear();
    ...
    while (it2.isNext()) {
        ...
        if (up.isInside(xp) == true) {
            if (left.isInside(xp) == true) {
                ...
                boundary_without_corner.add();
                boundary_without_corner.last().get<0>() = p.getKey();
            }
            boundary.add();
...
/*                                          pol                             V         vort                 Ext    Press     strain       stress                      Mfield,   dPol                      dV         RHS                  f1     f2     f3    f4     f5     f6       H               V_t      div    H_t                                                                                      delmu */
        ...
        openfpm::vector<aggregate<int>> boundary_without_corner;
...
*/
        ...
        vector_dist_subset<2, double, aggregate<VectorS<2, double>, VectorS<2, double>, double[2][2], VectorS<2, double>, double, double[2][2], double[2][2], VectorS<2, double>, VectorS<2, double>, VectorS<2, double>, VectorS<2, double>, double, double, double, double, double, double, double, VectorS<2, double>, double, double, double[2], double[2], double[2], double[2], double[2], double[2], double, VectorS<2, double>, VectorS<2, double>, VectorS<2, double>, VectorS<2, double>, double, double, double>>Particles_subset(Particles,bulk);
        auto P_bulk = getV<Pressure>(Particles_subset);//Pressure only on inside
        ...
        while (tim <= tf) {
            ...
            indexUpdate(Particles, Particles_subset, boundary,boundary_without_corner, corner_ul,
