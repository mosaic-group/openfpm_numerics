...
*/
    ...
    */
    ...
    BOOST_AUTO_TEST_CASE(solver_ana_stag)
    {
        ...
        // Distributed grid that store the solution
        staggered_grid_dist<2,float,aggregate<Point<2,float>,float,Point<2,float>,float,float>> g_dist(szu,domain,g);
        grid_dist_id<2,float,aggregate<Point<2,float>,float,Point<2,float>,float,float>> g_dist_normal(g_dist.getDecomposition(),szu,g);
        ...
        // Finite difference scheme
        FD_scheme<ana_nn_stag,decltype(g_dist)> fd(pd,stencil_max,g_dist);

        auto f =  FD::getV_stag<4>(g_dist);
        auto P =  FD::getV_stag<1>(g_dist);
        auto v = FD::getV_stag<0>(g_dist);
        ...
        auto Stokes_vx = nu*Lap(v[x]) - Dx(P) + f;
        auto Stokes_vy = nu*Lap(v[y]) - Dy(P) + f;
        ...
        // fd.impose(v[x], {0,0},{0,sz[1]-2},Vx,vx,left_cell);
        ...
        // fd.impose(v[y], {-1,0},{-1,sz[1]-1},Vy,vy,corner_right);
        ...
        // fd.impose(v[x],{sz[0]-1,0},{sz[0]-1,sz[1]-2},Vx,vx,left_cell);
        for (int j = 0; j <= sz[1]-2 ; ++j)
        {
            double X = (sz[0]-1) * hx;
            double Y = j * hy;
        ...
        // // Imposing B3 Top Wall
        // fd.impose(v[x], {0,-1},{sz[0]-1,-1},Vx,vx,corner_up);
        ...
        // fd.impose(v[y], {0,0},{sz[0]-2,0},Vy,vy,bottom_cell);
        ...
        // Imposing B4 Bottom Wall
        // fd.impose(v[x],{0,sz[1]-1},{sz[0]-1,sz[1]-1},Vx,vx,corner_dw);
        for (int i = 0; i <= sz[0]-1 ; ++i)
        {
            double X = i * hx;
            double Y = (sz[1]-1) * hy;
        ...
        // fd.impose(v[y],{0,sz[1]-1},{sz[0]-2,sz[1]-1},Vy,vy,bottom_cell);
        for (int i = 0; i <= sz[0]-2 ; ++i)
        {
            double X = i * hx;
            double Y = (sz[1]-1) * hy;
        ...
        // fd.impose(v[x], {-1,-1},{-1,sz[1]-1},Vx,vx,left_cell);
        ...
        // fd.impose(v[y], {-1,-1},{sz[0]-1,-1},Vy,vy,bottom_cell);
