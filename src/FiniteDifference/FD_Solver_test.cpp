...
/*        Solver.impose(Lap(v),{1,1},{79,79}, prop_id<1>());
    ...
    {using namespace FD;
        ...
        while (V_err >= V_err_eps && n <= nmax) {
            ...
            Solver.impose(StokesX, {1,1},{80,80}, prop_id<4>(), vx);
            div=-1.0*RHS[y];
            Solver.impose(StokesY, {1,1},{80,80}, prop_id<4>(), vy);
            Solver.impose(V_star[x], {0,81},{81,81}, 1.0, vx);
            Solver.impose(V_star[y], {0,81},{81,81}, 0.0, vy);
            Solver.impose(V_star[x], {0,1},{0,80}, 0.0, vx);
            Solver.impose(V_star[y], {0,1},{0,80}, 0.0, vy);
            Solver.impose(V_star[x], {81,1},{81,80}, 0.0, vx);
            Solver.impose(V_star[y], {81,1},{81,80}, 0.0, vy);
            Solver.impose(V_star[x], {0,0},{81,0}, 0.0, vx);
            Solver.impose(V_star[y], {0,0},{81,0}, 0.0, vy);
            auto A=Solver.getA();
            A.write("Matrix");
            ...
            SolverH.impose(Helmholtz,{1,1},{80,80},prop_id<4>());
            SolverH.impose(Dy(H), {0,81},{81,81},0);
            SolverH.impose(Dx(H), {81,1},{81,80},0);
            SolverH.impose(Dx(H), {0,0},{81,0},0,vx,true);
            SolverH.impose(H, {0,0},{0,0},0);
            SolverH.impose(Dy(H), {0,1},{0,80},0);
